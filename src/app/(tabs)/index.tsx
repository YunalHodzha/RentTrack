  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: bg }} contentContainerStyle={{ paddingBottom: 40 }}>
      <View style={{ paddingTop: 60, paddingHorizontal: 20, paddingBottom: 20, backgroundColor: card, borderBottomWidth: 1, borderBottomColor: border }}>
        ...
        <Text style={{ fontSize: 28, fontWeight: '700', color: text, marginTop: 2 }}>Dashboard</Text>
      </View>

      <View style={{ padding: 20, gap: 16 }}>
        <View style={{ flexDirection: 'row', gap: 12 }}>
          <StatCard title="Monthly Income" value={`€${monthlyIncome.toFixed(0)}`} sub={`${activeLeases.length} active leases`} color="#2563EB" card={card} text={text} sub2={sub} border={border} />
          <StatCard title="Occupancy" value={`${occupancyRate}%`} sub={`${rentedCount} / ${props.length} properties`} color="#16A34A" card={card} text={text} sub2={sub} border={border} />
        </View>

        <View style={{ flexDirection: 'row', gap: 12 }}>
          <StatCard title="Collected" value={`€${collected.toFixed(0)}`} sub={currentPeriod} color="#16A34A" card={card} text={text} sub2={sub} border={border} />
          <StatCard title="Outstanding" value={`€${owed.toFixed(0)}`} sub={currentPeriod} color={owed > 0 ? '#D97706' : '#16A34A'} card={card} text={text} sub2={sub} border={border} />
        </View>

        {overdueLeases.length > 0 && (
          <View style={{ backgroundColor: '#FEF3C7', borderRadius: 12, padding: 16, borderWidth: 1, borderColor: '#FDE68A' }}>
            <Text style={{ fontSize: 14, fontWeight: '700', color: '#92400E' }}>⚠️ Overdue Payments</Text>
            <Text style={{ fontSize: 13, color: '#92400E', marginTop: 4 }}>
              {overdueLeases.length} lease{overdueLeases.length > 1 ? 's' : ''} past payment day with no payment recorded
            </Text>
          </View>
        )}
        ...
        {props.length === 0 && (
          <View style={{ backgroundColor: card, borderRadius: 12, padding: 24, alignItems: 'center', borderWidth: 1, borderColor: border }}>
            ...
            <Text style={{ fontSize: 16, fontWeight: '600', color: text, marginTop: 12 }}>Get started</Text>
            <Text style={{ fontSize: 14, color: sub, marginTop: 6, textAlign: 'center' }}>
              Add your first property in the Properties tab
            </Text>
          </View>
        )}
      </View>
