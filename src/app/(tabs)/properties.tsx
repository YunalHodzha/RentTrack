    ...
    <View style={{ backgroundColor: card, borderRadius: 12, padding: 16, marginBottom: 12, borderWidth: 1, borderColor: border }}>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <View style={{ flex: 1 }}>
          ...
          <Text style={{ fontSize: 12, color: sub, marginTop: 4, textTransform: 'capitalize' }}>{item.type}</Text>
        </View>
  ...
  return (
    <View style={{ flex: 1, backgroundColor: bg }}>
      <View style={{ paddingTop: 60, paddingHorizontal: 20, paddingBottom: 16, backgroundColor: card, borderBottomWidth: 1, borderBottomColor: border }}>
        <Text style={{ fontSize: 28, fontWeight: '700', color: text }}>Properties</Text>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={{ alignItems: 'center', paddingTop: 80 }}>
            ...
            <Text style={{ fontSize: 18, fontWeight: '600', color: text, marginTop: 16 }}>No properties yet</Text>
...
function AddPropertyModal({ visible, onClose, onSave, isDark }: {
  ...
  function handleSave() {
    if (!name.trim()) { Alert.alert('Required', 'Please enter a property name.'); return; }
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet">
      <View style={{ flex: 1, backgroundColor: bg }}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 20, paddingTop: 24, borderBottomWidth: 1, borderBottomColor: border }}>
          ...
          <Text style={{ fontSize: 18, fontWeight: '600', color: text }}>Add Property</Text>
        ...
        <ScrollView contentContainerStyle={{ padding: 20 }}>
          <Field label="Name *" isDark={isDark}>
            <TextInput value={name} onChangeText={setName} placeholder="e.g. Apt 3, 8th Street" placeholderTextColor={sub} style={{ backgroundColor: inputBg, borderRadius: 10, padding: 12, color: text, fontSize: 16 }} />
          </Field>

          <Field label="Address" isDark={isDark}>
            <TextInput value={address} onChangeText={setAddress} placeholder="Optional" placeholderTextColor={sub} style={{ backgroundColor: inputBg, borderRadius: 10, padding: 12, color: text, fontSize: 16 }} />
          </Field>
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
              {PROPERTY_TYPES.map((t) => (
                <TouchableOpacity
                  ...
                  <Text style={{ color: type === t ? '#fff' : text, fontSize: 14, textTransform: 'capitalize' }}>{t}</Text>
                </TouchableOpacity>
              ))}
            </View>
          </Field>
